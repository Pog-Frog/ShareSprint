...
export class UserService{
    ...
    public async findUserById(userId: string): Promise<User>{
        const findUser: User = await UserModel.findById(userId).populate('posts').populate('comments').populate('likes').populate('notifications');
        if(!findUser) throw new HttpException(409, "User not found");
    ...
    public async findUserByEmail(email: string): Promise<User>{
        const findUser: User = await UserModel.findOne({email: email}).populate('posts').populate('comments').populate('likes').populate('notifications');
        if(!findUser) throw new HttpException(409, "User not found");
